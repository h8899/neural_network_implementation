...
def eval_numerical_gradient_inputs(layer, inputs, in_grads, h=1e-5):
    ...
    for i in range(len(inputs)):
        ...
        it = np.nditer(inputs[i], flags=['multi_index'], op_flags=['readwrite'])
        while not it.finished:
            ...
            grads[i][idx] = np.sum(np.nan_to_num(pos - neg) * in_grads) / (2 * h)
            it.iternext()
...
def check_grads(cacul_grads, numer_grads, threshold=1e-7):
    precise = np.linalg.norm(cacul_grads-numer_grads)/max(max(np.linalg.norm(cacul_grads), np.linalg.norm(numer_grads)), threshold)
    return precise
...
def check_grads_layer(layer, inputs, in_grads):
    ...
    if isinstance(cacul_grads, list):
        for i in range(len(cacul_grads)):
            ...
            print('Gradients to inputs {}: {}'.format(i, inputs_result))
    else:
    ...
    if layer.trainable:
        ...
        for k, v in cacul_grads.items():
            ...
            print('Gradients to {}: {}'.format(k, results))
...
def check_grads_loss(layer, inputs, targets):
    ...
    if isinstance(cacul_grads, list):
        for i in range(len(cacul_grads)):
            inputs_result = check_grads(cacul_grads[i], numer_grads[i])
            print('inputs {}: {}'.format(i, inputs_result))
    else:
        inputs_result = check_grads(cacul_grads, numer_grads)
